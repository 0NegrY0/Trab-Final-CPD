#include "../include/TabelaHash.h"

#include <iostream>

//////////////////////////////////
/// CONSTRUTORES E DESTRUTORES ///
//////////////////////////////////

TabelaHash_EndAberto::TabelaHash_EndAberto(unsigned int m):
M(m)
{
    tabela.resize(M);
    for(unsigned int i=0;i<M;i++)
    {
        tabela[i].usado=false;
        tabela[i].ocupado=false;
        tabela[i].registro=NULL;
    }

    totalColisoes=0;
}

TabelaHash_EndFechado::TabelaHash_EndFechado(unsigned int m, unsigned int t):
M(m), TamanhoBucket(t)
{
    tabelaChaining.resize(M);

    tabelaBuckets.resize(M);
    for(unsigned int i=0;i<M; i++)
        tabelaBuckets[i].resize(TamanhoBucket);

    totalColisoes=0;
}

TabelaHash_EndAberto::~TabelaHash_EndAberto()
{

}

TabelaHash_EndFechado::~TabelaHash_EndFechado()
{

}

////////////////////////////
/// ENDERECAMENTO ABERTO ///
////////////////////////////

bool TabelaHash_EndAberto::insereEABL(std::string chave, Registro* reg)
{
    int colisoes = 0;
    bool inseriu = false;

    ///TODO: Completar funcao de insercao em tabela de enderecamento aberto com busca linear
    int e, marca, endc, endL;
    e = -1;
    marca = -1;
    endc = MapearChavesA(chave, M);
    endL = endc;

    do{
        if(tabela[endL].usado == true)
            if(tabela[endL].ocupado == true)
                if(tabela[endL].chave == chave){
                    e = endL;
                    break;
                }
                else{
                    endL = (endL + 1)%M;
                    colisoes++;
                }
            else{
                marca = endL;
                break;
            }
        else{
            marca = endL;
            break;
        }
    }while(endL != endc || e == -1);

    if(marca != -1 && e == -1){
        inseriu = true;
        tabela[marca].chave = chave;
        tabela[marca].registro = reg;
        tabela[marca].usado = true;
        tabela[marca].ocupado = true;
    }

    totalColisoes += colisoes;

    return inseriu;
}

Registro* TabelaHash_EndAberto::buscaEABL(std::string chave)
{
    Registro* r = NULL;

    ///TODO: Completar funcao de busca em tabela de enderecamento aberto com busca linear

    int endc, endL;
    endc = MapearChavesA(chave, M);
    endL = endc;

    do{
        if(tabela[endL].usado == true)
            if(tabela[endL].ocupado == true && tabela[endL].chave == chave){
                r = tabela[endL].registro;
                break;
            }
            else{
                endL = (endL + 1)%M;
                if(endL == endc)
                    break;
            }
        else
            break;
    }while(endL != endc || r == NULL);

    return r;
}

bool TabelaHash_EndAberto::insereDoubleHashing(std::string chave, Registro* reg)
{
    int colisoes=0;
    bool inseriu = false;

    ///TODO: Completar funcao de insercao em tabela de enderecamento aberto com double hashing
    int j, e, marca, endc, endL;
    j = 0;
    e = -1;
    marca = -1;
    endc = MapearChavesA(chave, M);
    endL = endc;

    do{
        if(tabela[endL].ocupado == true){
            if(tabela[endL].chave == chave){
                e = endL;
                inseriu = true;
                break;
            }
            else{
                colisoes++;
                j++;
                for(int i = 0; i < chave.size(); i++){
                    endL = (endc + 23 * j * endL + chave[i]) % M;
                }
                if(endL >= M){
                    endL -= M;
                }
                if(endL == endc && j > M){
                    break;
                }
            }
        }
        else{
            if(marca == -1){
                marca = endL;
                break;
            }
        }
    }while(endL != endc || e == -1);

    if(marca != -1 && e == -1){
        inseriu = true;
        tabela[marca].chave = chave;
        tabela[marca].registro = reg;
        tabela[marca].usado = true;
        tabela[marca].ocupado = true;
    }

    totalColisoes += colisoes;

    return inseriu;
}

Registro* TabelaHash_EndAberto::buscaDoubleHashing(std::string chave)
{
    Registro* r = NULL;

    ///TODO: Completar funcao de busca em tabela de enderecamento aberto com double hashing

    int j, endc, endL;
    j = 0;
    endc = MapearChavesA(chave, M);
    endL = endc;

    do{
        if(tabela[endL].usado == true)
            if(tabela[endL].ocupado == true && tabela[endL].chave == chave){
                r = tabela[endL].registro;
                break;
            }
            else{
                j++;
                for(int i = 0; i < chave.size(); i++)
                    endL = (endc + 23 * j * endL + chave[i]) % M;
                if(endL >= M)
                    endL -= M;
                if(endL == endc && j > M)
                    break;
            }
        else
            break;
    }while(endL != endc || r == NULL);

    return r;
}

int TabelaHash_EndAberto::colisoesOcorridas()
{
    return totalColisoes;
}

/////////////////////////////
/// ENDERECAMENTO FECHADO ///
/////////////////////////////

bool TabelaHash_EndFechado::insereChaining(std::string chave, Registro* reg)
{
    int colisoes=0;
    bool inseriu = false;

    ///TODO: Completar funcao de insercao em tabela de enderecamento fechado com encadeamento (chaining)

	int end = 0;
    int aux = -1;
    int inserir = 0;

    //Criando buffer
    
    Elemento buf;
    buf.chave = chave;
    buf.ocupado = true;
    buf.registro = reg;

    /*for(unsigned int i = 0; i < chave.size(); i++)
        end = (chave[i] + (31 * end) % tabelaChaining.size());
   */
    end = MapearChavesF(chave, tabelaChaining.size());
    

    if (tabelaChaining[end].empty() != true)
        for(std::list<Elemento>::iterator iter = tabelaChaining[end].begin(); iter != tabelaChaining[end].end(); ++iter){
            if(inserir == 0){
                if(buf.chave == iter->chave){
                    aux = 0;
                    inserir = 1;
                }
                else
                    colisoes++;
            }
            else{
                inserir = 0;
                break;
            }
        }
        
    if(aux == -1)
        tabelaChaining[end].push_back(buf);

    inseriu = true;
    totalColisoes = totalColisoes + colisoes;

    totalColisoes += colisoes;

    return inseriu;
}

Registro* TabelaHash_EndFechado::buscaChaining(std::string chave)
{
    Registro* r = NULL;

    ///TODO: Completar funcao de busca em tabela de enderecamento fechado com encadeamento (chaining)

	int endC = 0;

    endC = MapearChavesF(chave, tabelaChaining.size());

    int endL = endC;


    if(tabelaChaining[endL].empty() == true)
        return r;
    else{
        std::list<Elemento>::iterator iter = tabelaChaining[endL].begin();

        if(iter->chave == chave){
            r = iter -> registro;
            return r;
        }

        while (iter != tabelaChaining[endL].end()){
            if(iter->chave == chave){
                r = iter->registro;
                return r;
            }
            iter++;
        }
    }


    return r;
}

bool TabelaHash_EndFechado::insereBuckets(std::string chave, Registro* reg)
{
    int colisoes=0;
    bool inseriu = false;

    ///TODO: Completar funcao de insercao em tabela de enderecamento fechado com blocos de tamanho fixo (buckets) e area de overflow
	int i;                      //maybe unsigned
    int end = 0;
    int size;

    Elemento buf;
    buf.chave = chave;
    buf.ocupado = true;
    buf.registro = reg;

    end = MapearChavesF(chave, tabelaBuckets.size());

    size = tabelaBuckets[end].size();

    if(tabelaBuckets[end][0].ocupado == true){
        for(i = 0; i < size; i++){
            if(tabelaBuckets[end][i].chave != buf.chave){
                colisoes++;

                if(tabelaBuckets[end][i].ocupado == false){
                    tabelaBuckets[end][i] = buf;
                    inseriu = true;
                    break;
                }
            }
            else{
                inseriu = true;
                break;
            }
        }

        if(i == size)
            overflow.push_back(buf);
    }

    else{
        tabelaBuckets[end][0] = buf;
        inseriu = true;
    }  



    totalColisoes += colisoes;

    return inseriu;
}

Registro* TabelaHash_EndFechado::buscaBuckets(std::string chave)
{
    Registro* r = NULL;

    ///TODO: Completar funcao de busca em tabela de enderecamento fechado com blocos de tamanho fixo (buckets) e area de overflow

	int endC = 0;

    endC = MapearChavesF(chave, tabelaBuckets.size());

    int endL = endC;

    std::vector<Elemento>::iterator iter = tabelaBuckets[endL].begin();

    if(iter -> chave == chave){
        r = iter->registro;
        return r;
    }

    while(iter != tabelaBuckets[endL].end()){
        if(iter->chave == chave){
            r = iter->registro;
            return r;
        }
        iter++;
    }




    return r;
}

int TabelaHash_EndFechado::colisoesOcorridas()
{
    return totalColisoes;
}

int TabelaHash_EndAberto::MapearChavesA(std::string s, int M){
    int hash = 0;
    for (int i = 0; i < s.length(); i++)
        hash = (31 * hash + s.at(i)) % M;
    return hash;
}

int TabelaHash_EndFechado::MapearChavesF(std::string s, int M){
    int hash = 0;
    for (int i = 0; i < s.length(); i++)
        hash = (31 * hash + s.at(i)) % M;
    return hash;
}
