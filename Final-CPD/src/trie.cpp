...
void Trie::iniciaTrie(){
    fstream trieBin;
    trieBin.open("trieBin.bin"); //, ios::out | ios::binary); // | ios::in

    if(!trieBin.is_open()){
        cerr << "Could not open the file - 'trieBin.bin'" << endl;
        exit(EXIT_FAILURE);
    }
}

void Trie::saveTrie(char name[NAME_MAX], /*string name,*/ long position, fstream trieBin){
...
///---------------------------------------------------------------------------------------------------
///---------------------------------------------------------------------------------------------------
void Trie::searchByName(char nomeProcurado[NAME_MAX], fstream trieBin){
