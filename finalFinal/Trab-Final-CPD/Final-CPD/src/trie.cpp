...
void searchByIssue(char nomeProcurado[], fstream& trieBin, fstream& regBin){
    ...
    if(flag == 1){
        cout << "Marvel Comics nao encontrado" << endl ;
    }
    // Senao, os proximos nodos sao percorridos em busca de todos os filmes que possuam tal prefixo ou nome
    else{
        trieBin.seekg(search_node.son_pos, trieBin.beg);
        teste(search_node, trieBin, regBin);
    }
}
...
void teste(Trie_node search_node, fstream& trieBin, fstream& regBin){
    ...
    if(search_node.file_position != -1){
        regBin.seekg(search_node.file_position, regBin.beg);
        regBin.read((char*)&reg, sizeof(Registro));
        printReg(reg);
    }
    // Continua a busca pelo nodo a esquerda
    if(search_node.left_pos != -1){
        trieBin.seekg(search_node.left_pos, regBin.beg);
        teste(search_node, trieBin, regBin);
    }

    // Continua a busca pelo filho
    if(search_node.son_pos != -1){
        trieBin.seekg(search_node.son_pos, regBin.beg);
        teste(search_node, trieBin, regBin);
    }

    // Continua a busca pelo nodo a direita
    if(search_node.right_pos != -1){
        trieBin.seekg(search_node.right_pos, regBin.beg);
        teste(search_node, trieBin, regBin);
    }
